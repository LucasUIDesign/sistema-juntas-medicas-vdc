...
El sistema de Juntas Médicas maneja **4 estados diferentes** para el ciclo de vida de una junta médica.
...
Médico presiona "Finalizar" → Estado: PENDIENTE
```
...
### 2. 🟡 **PENDIENTE**
- **Color**: Amarillo
- **Significado**: La junta ha sido finalizada por el médico y está esperando revisión del Director Médico
- **Quién lo asigna**: Sistema (cuando médico finaliza o cuando admin asigna turno)
- **Cuándo se usa**:
  - Cuando el médico completa y finaliza el dictamen
  - Cuando el admin asigna un turno a un médico
  - La junta está esperando aprobación o rechazo del Director
- **Acciones permitidas**:
  - ✅ Director Médico puede revisar
  - ✅ Director Médico puede aprobar o rechazar
  - ✅ Médico puede subir documentos faltantes
  - ✅ Visible en dashboard de Director Médico
  - ❌ Médico no puede editar el dictamen (ya finalizado)
...
**Nota importante:** Los documentos faltantes se muestran dentro de los detalles de la junta, no hay un estado separado para esto.
...
**Ejemplo de uso:**
```
Director revisa junta PENDIENTE → Aprueba → Estado: APROBADA
```
...
**Ejemplo de uso:**
```
Director revisa junta PENDIENTE → Encuentra errores → Rechaza → Estado: RECHAZADA
```
...
1. PENDIENTE (admin asigna turno)
   ↓
2. PENDIENTE (médico completa - se mantiene)
   ↓
3. APROBADA (director aprueba)
```
...
- ✅ Puede finalizar juntas (BORRADOR → PENDIENTE)
- ✅ Puede subir documentos en estado PENDIENTE
- ❌ No puede aprobar/rechazar
- ❌ No puede editar después de finalizar

### Director Médico
- ✅ Puede revisar juntas PENDIENTES
- ✅ Puede aprobar (PENDIENTE → APROBADA)
...
2. **El médico NO puede editar** una junta después de finalizarla (PENDIENTE)
3. **Los documentos faltantes** se muestran en los detalles de la junta, no hay estado separado
4. **BORRADOR** no es visible para el Director Médico
5. **APROBADA y RECHAZADA** son estados finales (no se pueden cambiar)
6. **Cuando el médico finaliza**, el estado pasa automáticamente a PENDIENTE

---

## 🔍 Consultas Comunes

### ¿Cuándo una junta pasa de BORRADOR a PENDIENTE?
Cuando el médico presiona el botón "Finalizar Junta" después de completar el dictamen.

### ¿Puede un médico editar una junta PENDIENTE?
No, una vez finalizada (PENDIENTE), el médico no puede editar el dictamen. Solo puede subir documentos faltantes. El Director puede aprobar o rechazar.

### ¿Qué pasa si faltan documentos?
Los documentos faltantes se muestran dentro de los detalles de la junta. El médico puede subirlos mientras la junta está en estado PENDIENTE. No hay un estado separado para documentos pendientes.

### ¿Puede una junta RECHAZADA volver a PENDIENTE?
...
Solo el médico que la creó. No es visible para Director Médico ni RRHH hasta que se finalice y pase a PENDIENTE.

### ¿Cuál es la diferencia entre PENDIENTE cuando el admin asigna vs cuando el médico finaliza?
Es el mismo estado. PENDIENTE significa que la junta está esperando ser completada (si fue asignada por admin) o esperando revisión (si fue finalizada por el médico). En ambos casos, el estado es PENDIENTE.

---

## 📋 Cambios Recientes

**Actualización: 30 de Enero de 2026**
- ❌ Eliminado estado COMPLETADA (ahora se usa PENDIENTE)
- ❌ Eliminado estado DOCUMENTOS_PENDIENTES (se muestra en detalles)
- ✅ Simplificado a 4 estados: BORRADOR, PENDIENTE, APROBADA, RECHAZADA
- ✅ Cuando el médico finaliza → Estado: PENDIENTE
- ✅ Los documentos faltantes se muestran en los detalles de la junta
